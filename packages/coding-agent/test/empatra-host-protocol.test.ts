...
describe("Empatra host protocol", () => {
	test("advertises the closed versioned capability catalog and parses strict subsets", () => {
		...
		// All optional host-owned bridges may be negotiated together. Keep this
		// regression here because the parser must accept the complete closed
		// advertised set, not only the core catalog.
		const completeAdvertisedCatalog = [
			...EMPATRA_HOST_CAPABILITIES,
			EMPATRA_HOST_SUBAGENT_CAPABILITY,
			EMPATRA_HOST_FRAMING_CAPABILITY,
			EMPATRA_HOST_MCP_OAUTH_CAPABILITY,
		...
		expect(parseEmpatraHostCapabilities(completeAdvertisedCatalog)).toEqual(completeAdvertisedCatalog);
